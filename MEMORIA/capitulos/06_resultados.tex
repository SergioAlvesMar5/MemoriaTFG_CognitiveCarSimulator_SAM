...
La tabla~\ref{tab:comparacion-bloques-test} resume la secuencia de pruebas archivadas. No constituye un experimento controlado entre fechas, porque durante junio cambian el fitness, las intersecciones, el número de spawns y los modelos. Se utiliza para mostrar la evolución observada, no para atribuir causalidad a una modificación aislada. Sus filas agregan las sesiones de test archivadas por día; por ello, el 26 de junio combina varios bloques, mientras que el bloque alto de 177 evaluaciones, con 147 aciertos, se analiza de forma separada como contraste en la Tabla~\ref{tab:cambios-tecnicos-resultados}. En esta tabla, el fitness medio se calcula directamente sobre \texttt{Fitness\_Final}; por ello puede diferir ligeramente del promedio de \textit{MeanFit} de la tabla~\ref{tab:resultados-principales}. El duplicado exacto del 11 de junio se excluye. Cuando existen pequeños descuadres entre \texttt{Test\_Summary.csv} y \texttt{Fitness\_Debug.csv}, la tasa se calcula a partir de las evaluaciones declaradas en el resumen de test, mientras que la columna de evaluaciones puede reflejar los registros de detalle disponibles.
...
\mbox{\scriptsize\texttt{STOP\_VIOLATION\_TIMEOUT}} & Agotamiento del tiempo de validación de stop en la lógica posterior. & Fallo legal.\\
\shortstack[l]{\scriptsize\texttt{YIELD\_VIOLATION\_EXIT}\\\scriptsize\texttt{YIELD\_VIOLATION\_TIMEOUT}} & Salida indebida o espera agotada en ceda el paso. & Fallo legal.\\
...
\texttt{FLIPPED} & Vuelco físico del vehículo. & Fallo.\\
\texttt{INVALID\_BRAIN} & Modelo, red o pesos incompatibles tras las guardas de inferencia. & Fallo.\\
