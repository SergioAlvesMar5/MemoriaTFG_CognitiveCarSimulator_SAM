...
La planificación real del proyecto puede entenderse como un diagrama de Gantt incremental. Las primeras semanas estuvieron dedicadas a comprender el proyecto heredado, ordenar assets y crear una primera señalización funcional. Después se concentró el esfuerzo en el vehículo autónomo, el entrenamiento evolutivo y el entorno de pruebas. A partir de marzo el trabajo se desplazó hacia la instrumentación, la separación entre entrenamiento y test, las intersecciones, la señalización avanzada y, finalmente, la interacción física entre vehículos.

{\small
\begin{table}[htbp]
\centering
\caption{Diagrama de Gantt simplificado del desarrollo.}%
\label{tab:gantt}
\begin{tabularx}{0.98\textwidth}{L{2.55cm}L{1.55cm}L{1.15cm}X}
\toprule
\textbf{Fase} & \textbf{Periodo} & \textbf{Esf.} & \textbf{Resultado principal}\\
\midrule
Arranque y limpieza del proyecto & Nov.-dic. & \rule{0.9cm}{1.05ex} & Carpeta \texttt{CarSimulator}, assets ordenados y revisión del estado heredado.\\
Señalización y vehículo inicial & Dic.-ene. & \rule{0.9cm}{1.05ex} & Semáforos, señales verticales, primer coche con sensores y salidas continuas.\\
Línea de pruebas y aprendizaje básico & Ene.-feb. & \rule{0.9cm}{1.05ex} & Entrenamiento de centrado, corrección de desviación, curvas y primeros checkpoints.\\
Spawns, bordes e instrumentación & Feb.-mar. & \rule{0.9cm}{1.05ex} & Puntos de aparición distribuidos, bordes virtuales y primeros CSV de diagnóstico.\\
Train/test y señalización en la red & Mar.-abr. & \rule{0.9cm}{1.05ex} & Separación de modos, contexto one-hot, zonas de espera y cruce.\\
Fitness avanzado y persistencia & Abr.-may. & \rule{0.9cm}{1.05ex} & Penalizaciones, modelos persistentes, elitismo y mutación dinámica.\\
Rotondas y análisis de regresión & Jun. & \rule{0.55cm}{1.05ex} & Métricas por salida, balances de recompensa y test reproducibles.\\
Interacción física entre vehículos & Jun.-jul. & \rule{0.9cm}{1.05ex} & Solapes, distancia de seguridad, cedas con cola y simulación libre con respawn.\\
\bottomrule
\end{tabularx}
\caption*{\textit{\textbf{Fuente.} Elaboración propia a partir de actas, milestones, issues y descripciones fechadas.}}
\end{table}
}
...
3 & 25/12/2025 a 08/01/2026 & 2 & Milestone redefinida como \textit{Gestor evolutivo, persistencia y restricciones de conducción}. Agrupa el sistema evolutivo de evaluación/guardado y las restricciones de velocidad, dirección, marcha atrás y centrado en carril.\\
4 & 08/01 a 16/01/2026 & 2 & Milestone redefinida como \textit{Validación inicial del modelo y documentación de prácticas}. Agrupa los primeros ciclos train/test sobre desviación y carril junto con la memoria de prácticas externas.\\
...
\section{Entorno de pruebas inicial}

Una parte decisiva del desarrollo fue construir una línea de pruebas pequeña antes de intentar que los vehículos circularan por toda la ciudad. Esa zona inicial reducía deliberadamente el problema: un mismo punto de aparición, carril reconocible, bordes controlados, checkpoints simples y una función de fitness centrada en avanzar, mantenerse dentro del carril, corregir desviaciones y aprender a girar. El objetivo no era representar todavía todo el tráfico urbano, sino aislar los conceptos mínimos que un coche físico necesitaba dominar antes de enfrentarse a intersecciones y señales.

Esta decisión evitó mezclar demasiadas causas de fallo en la misma prueba. Cuando el coche no giraba o se salía del carril, la explicación estaba en sensores, red, suavizado o fitness, no en una rotonda mal delimitada o en un stop situado demasiado cerca del spawn. Por eso la línea inicial funcionó como un banco de ensayo: permitió comprobar la conexión entre entradas, pesos, salidas y físicas de Chaos Vehicle, y también obligó a abandonar fórmulas de fitness que premiaban estrategias visualmente pobres.

El salto posterior a puntos de aparición dinámicos alrededor del mapa cambió el problema de escala. El gestor dejó de evaluar una habilidad local y pasó a distribuir la población entre zonas heterogéneas, con curvas, cruces, semáforos, stops y cedas. La introducción de \texttt{SpawnBag}, marcas \texttt{ValidForTrain}/\texttt{ValidForTest} y normalización por spawn convirtió el entrenamiento en un proceso mucho más representativo, pero también más exigente: una mejora en un punto fácil podía dejar de ser dominante si no se sostenía en puntos de mayor dificultad. Ese cambio revolucionó el manager evolutivo porque obligó a seleccionar modelos por robustez espacial, no solo por el máximo de fitness alcanzado en una línea controlada.
...
\section{Complejidad del cambio de mapa}

El cambio o ampliación de mapa no fue un simple problema de que la red neuronal tuviera que aprender curvas nuevas. La dificultad temporal más importante estuvo en preparar manualmente la geometría útil para la percepción: colocar bordes de calzada, volúmenes de navegación, triggers de intersección, cajas de espera, zonas de cruce, flechas de parada y propiedades específicas de cada actor. Si un borde quedaba asociado a la dirección equivocada, si un volumen se solapaba con una zona válida o si una línea de stop no estaba bien orientada, el coche recibía una lectura incoherente aunque la política neuronal fuese razonable.

Esta carga de configuración explica por qué algunas regresiones se concentraron en actores concretos, como bordes de intersección o \texttt{NavModifierVolume}\@. La red podía responder de forma aceptable ante una curva o rotonda nueva cuando los sensores recibían límites consistentes; en cambio, una propiedad mal asignada en el escenario convertía el entrenamiento en ruido. Por ello, el trabajo de mapa debe considerarse parte del sistema de IA\@: define qué percibe el agente, qué cuenta como salida de calzada, qué trayectoria está habilitada y cuándo una espera se valida como legal. En futuras ampliaciones convendría automatizar parte de esta configuración o crear herramientas de verificación visual antes de lanzar entrenamientos largos.
...
Configuración manual del mapa & Lecturas sensoriales incoherentes y regresiones localizadas & Alta & Revisar propiedades de bordes, volúmenes, triggers y líneas de parada antes de cada entrenamiento largo; documentar hotspots por actor.\\
